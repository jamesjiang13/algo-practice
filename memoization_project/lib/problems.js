...
function lucasNumberMemo(n, memo = {}) {
    if (memo[n]) return memo[n]; // return previously calculated result
    if (n === 0) return 2; //define two base cases
    if (n === 1) return 1;
    memo[n] = lucasNumberMemo(n-1, memo) + lucasNumberMemo(n-2, memo) // add to memo obj as such {n: result-of-n}
    return memo[n] 
}
...
function minChange(coins, amount, memo = {}) {
    ...
    if (amount === 0) return 0; // base case, need no change = get no change;
